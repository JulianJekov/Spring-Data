...
//    Set<Category> getRandomEntity(long rand);
    ...
    @Query("select new org.example._09_xmlprocessingexercises.domain.dtos.category.CategoryByProductsCountDto" +
            "(c.name, count(p.id), avg (p.price), sum(p.price))" +
            "from Product p join p.categories c group by c.id order by count (p.id) desc")
